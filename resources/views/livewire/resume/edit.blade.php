<form wire:submit.prevent="submit" class="pt-3">
    ...
    <div class="form-group {{ $errors->has('resume.title') ? 'invalid' : '' }}">
        ...
        <input class="form-controle mt-1 block w-full border-none bg-gray-100 h-11 rounded-full text-center shadow-sm hover:bg-blue-100 focus:bg-blue-100 focus:ring-0" type="text" name="title" id="title" required wire:model.defer="resume.title">
    ...
    <div class="form-group {{ $errors->has('mediaCollections.resume_resume') ? 'invalid' : '' }}">
        ...
        <x-dropzone id="resume" name="resume" style="border: none; --tw-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05);
        box-shadow: var(--tw-ring-offset-shadow, 0 0 #0000), var(--tw-ring-shadow, 0 0 #0000), var(--tw-shadow); background-color: rgb(243 244 246);" action="{{ route('admin.resumes.storeMedia') }}" collection-name="resume_resume" max-file-size="5" max-files="1" />
        <div class="validation-message">
